...
- \`/ws/terminal/{nodeId}\`, \`/ws/logs\`, \`/ws/broadcast\` — WebSocket-каналы только для админ-сессии (cookie).
        ...
        operations: {
            ...
            'GET /users/{userId}/traffic-history': {
                summary: 'Получить историю трафика пользователя',
                description: 'Возвращает плотные UTC-интервалы трафика Hysteria и Xray и разбивку по нодам за выбранный период. Граница `to` не включается. История хранится не менее 45 дней, начинается после развёртывания этой версии и не очищается сбросом накопительного счётчика. Требуется скоуп `users:read`.',
            },
