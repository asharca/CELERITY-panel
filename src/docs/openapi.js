...
For \`virtual\` you must additionally pass a non-empty \`virtual\` object.`,
            properties: {
                ...
                portRange: { type: 'string', example: '20000-50000', default: '', description: 'UDP port hopping range. Empty or omitted disables port hopping.' },
                statsHost: { type: 'string', example: '10.0.0.10', default: '', description: 'Panel-side management host for Hysteria stats requests. Empty or omitted uses `ip`; never published to subscriptions.' },
...
\`ip\` (kept from the existing document or supplied here) must be non-empty.`,
            properties: {
                name: { type: 'string' },
                ip: { type: 'string', description: 'Server IP or hostname. Required when the resulting type is not virtual.' },
                domain: { type: 'string' },
                sni: { type: 'string' },
                port: { type: 'integer' },
                portRange: { type: 'string', description: 'UDP port hopping range. Set to an empty string to disable port hopping.' },
                statsHost: { type: 'string', description: 'Panel-side management host for Hysteria stats requests. Empty uses `ip`; never published to subscriptions.' },
        ...
        schemas: {
            ...
            Node: {
                ...
                properties: {
                    ...
                    portRange:     { type: 'string', example: '20000-50000', description: 'UDP port hopping range; empty means disabled.' },
                    statsHost:     { type: 'string', example: '10.0.0.10', description: 'Panel-side management host for Hysteria stats requests. Empty uses `ip`; never published to subscriptions.' },
